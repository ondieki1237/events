    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4">We couldn't find the product you requested.</p>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          <div className="w-full md:w-1/2">
            {product.image ? (
              ...
              <div className="w-full h-64 bg-muted/20 rounded-lg" />
            )}
          </div>

          <div className="w-full md:w-1/2">
            <h1 className="text-3xl font-bold mb-2">{product.title}</h1>
            <p className="text-sm text-muted-foreground mb-4">{product.category} • {product.impressions} impressions</p>
            <p className="text-base mb-6">{product.excerpt}</p>

            <div className="bg-white rounded-lg p-4 shadow-sm">
              <h4 className="font-semibold mb-2">Price</h4>
              <p className="text-sm text-foreground">{product.priceRange ?? 'Contact us for pricing'}</p>
              {product.brochure && (
                <p className="mt-3"><a className="text-primary" href={product.brochure} target="_blank" rel="noopener noreferrer">Download brochure</a></p>
              )}
            </div>
            ...
            <div className="mt-8">
              <Link href="/interest" className="inline-block bg-primary text-white px-4 py-2 rounded">Express Interest / Request Quote</Link>
            </div>
          </div>
        </div>
      </div>
